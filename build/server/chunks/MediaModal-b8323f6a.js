...
import { I as IconButton } from "./Page-961fbe05.js";
...
const BlogPost = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<article class="${"svelte-1d9ddim"}"><div class="${"article-body svelte-1d9ddim"}">${validate_component(SpacedDiv, "SpacedDiv").$$render($$result, {}, {}, {
    default: () => {
      return `<header class="${"svelte-1d9ddim"}"><h1 class="${"svelte-1d9ddim"}">${escape(meta.title)}</h1>
      <time${add_attribute("datetime", meta.date, 0)} class="${"svelte-1d9ddim"}">${escape(convertDate(meta.date))}</time></header>
